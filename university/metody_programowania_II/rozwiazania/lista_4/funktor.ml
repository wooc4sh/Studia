...
module type DICT = sig
  type ('a, 'b) dict (*Typ*)
  val empty : ('a, 'b) dict (*Slownik pusty*)
  val insert : 'a -> 'b -> ('a, 'b) dict -> ('a, 'b) dict (*Bierzemy jako argument:klucz, element oraz slownik, zwracamy slownik*)
  val remove : 'a -> ('a, 'b) dict -> ('a, 'b) dict (*Usuwamy element ze slownika o podanym kluczu.*)
  val find_opt : 'a -> ('a, 'b) dict -> 'b option (*Szukamy elementu w slowniku o podanym kluczu, jak nie ma to option*)
  val find : 'a -> ('a, 'b) dict -> 'b (*Zwykle poszukiwania bez option*)
  val to_list : ('a, 'b) dict -> ('a * 'b) list (*Bierzemy slownik, zwracamy liste par*)
end
...
(*Drzewo kodowe: w lisciach sa odpowiednie kody?*)
